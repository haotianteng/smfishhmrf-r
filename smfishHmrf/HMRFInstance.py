...
for(bx in 1:beta_num_iter){
    ...
    def evaluation(self,centroid_f,covariance_f,beta,iter_n = 10):
        cur = os.getcwd()
        os.chdir(self.dest)
        adjacency_file = os.path.join(self.dest,"f%s.adjacency.txt" % self.prefix)
        block_file = os.path.join(self.dest,"f%s.blocks.txt" % self.prefix)
        expr_file = os.path.join(self.dest,"f%s.expression.txt" % self.prefix)
        adjacency = np.asarray(pd.read_csv(adjacency_file,
                                           index_col = 0,
                                           delimiter = ' ',
                                           header = None))
        numnei = np.sum(adjacency!=-1,axis = 1)
        expression = np.asarray(pd.read_csv(expr_file,
                                            index_col = 0,
                                            delimiter = ' ',
                                            header = None))
        gene_n = expression.shape[1]
        mu = np.asarray(pd.read_csv(centroid_f,
                                    index_col = 0,
                                    delimiter = ' ',
                                    header = None))
        mu = np.transpose(mu)
        print(mu)
        cov = self.read_cov(covariance_f,gene_n)
        block = np.asarray(pd.read_csv(block_file,
                                       index_col = 0,
                                       delimiter =  ' ',
                                       header = None))
        block = block[:,0]
        maxblock = max(block)
        block = [np.where(block==x)[0]+1 for x in np.arange(1,maxblock+1)]
        ...
        rc = multi(y=expression, neighbors=adjacency, beta=beta, numnei=numnei, 
              ...
              err=1e-7, maxit=0,forceDetectDamp = True)
        os.chdir(cur)
        rc = dict(zip(rc.names, list(rc)))
        return np.array(rc['prob'])
    
    def read_cov(self,cov_file,gene_n):
        cov = np.zeros((gene_n,gene_n,self.k[0]))
        with open(cov_file,'r') as f:
            for i,line in enumerate(f):
                gene_i = i%gene_n
                k_i = i//gene_n
                cov[:,gene_i,k_i] = np.asarray([float(x) for x in line.split(' ')[1:]])
        return cov
